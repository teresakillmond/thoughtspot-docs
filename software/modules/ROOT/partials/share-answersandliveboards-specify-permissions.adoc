...
. If the selected group or user doesn't have access to the underlying data, you must enable access to the worksheet, view, or table.
A black warning symbol appears when you try to share with a user who doesn't have underlying data access.
...
If you don't own the data source, ThoughtSpot emails the owner of the data source or your ThoughtSpot administrator to ask them to share the data.
+
...
. To stop sharing with a user or group, click the *x* icon image:icon-delete-20px.png[Delete icon image].
+
...
+
Note that sending users this link doesn't share the object with them.
