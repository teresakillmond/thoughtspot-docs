...
[discrete]
ifndef::pendo-links[]
=== Conditional and number formatting for downloaded tables [.badge.badge-update]#Beta#
endif::[]
ifdef::pendo-links[]
=== Conditional and number formatting for downloaded tables [.badge.badge-update-whats-new]#Beta#
endif::[]

When you download a table in `XLSX` format, the downloaded table now shows the same conditional and number formatting as the table in ThoughtSpot. It also shows the same column width, number formatting based on the format patter, and date formatting as the table in ThoughtSpot. This feature is dependent on the new answer experience; to use this feature, you must first enable the new answer experience. For more information on downloaded tables,
ifndef::pendo-links[]
...
// ask manan if this needs answer v2 -- currently assuming it does
