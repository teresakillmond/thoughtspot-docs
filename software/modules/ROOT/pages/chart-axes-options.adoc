...
In ThoughtSpot, you can start changing all axes options by selecting the axis, directly in the chart.
...
To change the axis aggregation, select the *Aggregate* option in the axis menu, and select an alternative from the list of options.
...
. To add another condition, select *+add formatting*.
. To remove a defined conditional format, select the delete icon (*x*) next to its definition.
. Select *Done*.
...
Here, you can compare the original chart that wasn't sorted on the _Total Sales_ axis with the chart that uses descending sort.
