...
It is generally easier to interpret a chart if axes that use the same units of measure or scale appear on the same side of the chart.
...
The chart looks cleaner, and clearly communicates the distinct information on each of the two measures.
...
Format::
Category:::  This specifies the format of the axis measures.
