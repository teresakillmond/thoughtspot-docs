...
|A pinboard may have quite a large number of visualizations. When a user opens a pinboard, the system sends a request for several queries to render the first several visualizations on the screen. As the user scrolls downward in the pinboard, the system makes additional requests for queries that correspond to further visualizations and charts to render on the screen.
...
|Transforming a table in an Answer or inside a pinboard
...
|Schedule pinboard
|Multiple queries
|When a user schedules a pinboard without row-level security (RLS), the system generates the same number of queries required by the pinboard.

|Scheduling a pinboard, with RLS
|Multiple queries
|When a user schedules a pinboard with multiple row-level security rules, such as 2 RLS rules for 2 groups of users, the number of queries that the system generates depends on the number of user groups and the number of queries required by the pinboard.

|Adding a new chart to the pinboard
|Multiple queries
|When the user adds a new chart to the pinboard and then returns to the pinboard view through the link at bottom of the screen, the system generates the same number of initial queries as there are charts that appear in the viewport (on the screen) when the pinboard loads.

|Updating chart and return to pinboard
|Multiple queries
|When a user edits a chart and makes several changes, then updates or closes the chart and returns to the pinboard, the system generates queries for each change on enter during the editing time, and an additional query when the user saves the chart and returns to the pinboard.
...
|Downloading any answer as a chart inside a pinboard
|Multiple queries
|One (1) query for each chart Answer.

|Downloading pinboard as PDF
|Multiple queries
|The number of queries is equal to the sum of all queries generated from each tile in the pinboard.
|===
...
- Downloading a pinboard visualization as a table in any format
- Downloading an answer as a table in any format, NOT within a pinboard
- Searching answers
- Accessing system-default pinboards
- Importing TML of an answer or a pinboard
- Exporting TML of an answer or a pinboard without data (associated objects)
...
. Click the table in the pinboard, to open it in Answer mode.
...
== How do pinboards affect queries?

When you have a pinboard with a large number of visualizations, ThougtSpot does NOT generate all of them at the same time. Instead, the system only requests queries for the visualizations that appear in the initial viewport of the screen when it renders the pinboard.  As you scroll down, additional queries are sent to render the newly in-view visualizations.
So, the initial query request is approximately the same as the number of visualizations that appear in the pinboard before the user scrolls downward. Both tables and pivot tables require multiple queries, as described elsewhere in this article.
