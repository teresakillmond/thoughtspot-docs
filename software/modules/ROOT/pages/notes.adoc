...
NOTE: Self-service upgrade is not supported in version 7.1.
...
When connecting to remote files, DataFlow now automatically selects file format based on the source file type. For details, see xref:dataflow-files.adoc[Files].
DataFlow now supports the XML and Avro file formats.
