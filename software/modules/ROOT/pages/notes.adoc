...
For a complete list of functional issues that we fixed in this release, see xref:fixed.adoc#releases-7-1-x[Fixed issues].
...
New answer experience [.label.label-beta]#Beta#::
This release redesigns the experience of creating and working with answers. The new answer experience contains multiple new features and feature enhancements. It is in [.label.label-beta]#Beta# and off by default, but the option to turn it on or off individually is on by default. You can toggle it on or off individually from the *Experience manager* in your profile, or for the whole company by xref:support-contact.adoc[contacting ThoughtSpot Support].
+
...
Custom calendar::
Custom calendar is now available for Snowflake, Amazon Redshift, Google BigQuery, Microsoft Azure Synapse, and Databricks cloud data warehouses. See xref:connections-cust-cal.adoc[Custom calendar].
This feature is disabled by default. To enable it, xref:support-contact.adoc[contact ThoughtSpot Support].


AWS Secrets Manager for Redshift connection passwords::
Amazon Redshift connections now support AWS Secrets Manager for clusters deployed on an AWS EC2 instance. With this feature enabled, you no longer need to enter a password when you create a new Redshift connection. For details, see xref:connections-aws-secrets.adoc[Amazon Secrets Manager]. This feature is disabled by default. To enable it, xref:support-contact.adoc[contact ThoughtSpot Support].

[#it-ops-engineer-7-1-1]
=== For the IT Ops Engineer

Multiple Identity Providers for SAML login::
You can now configure SAML SSO login for more than one Identity Provider (IDP). You may have multiple groups of users who need to log into ThoughtSpot but are managed by separate IDPs. To configure this, refer to xref:saml.adoc#saml-configure-tscli[Configure SAML using tscli].
+
...
This release also introduces the Developer portal for application developers and ThoughtSpot Everywhere users. The portal provides a Playground for developer users to explore the SDK APIs and embedding workflows. The portal also offers several features, such as style customization and custom actions, to customize and enhance the embedded content.
To know more about ThoughtSpot Everywhere and learn how to embed ThoughtSpot content using Visual Embed SDK and REST APIs, go to *Develop* > *Guides*, or refer to the https://visual-embed-sdk-7-1-1.vercel.app/docs/[ThoughtSpot Everywhere documentation^].
