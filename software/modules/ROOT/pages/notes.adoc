...
Integration with AWS Secrets Manager for Redshift connection passwords::
Amazon Redshift connections now support AWS Secrets Manager for clusters deployed on an AWS EC2 instance. With this feature enabled, you no longer need to enter the password when you create a new Redshift connection. This feature is disabled by default. To enable it, xref:support-contact.adoc[contact ThoughtSpot Support].
