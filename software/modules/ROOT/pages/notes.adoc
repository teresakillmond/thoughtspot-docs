...
Data labels on a dark background  [.badge.badge-beta-relnotes]#Beta#::
Labels that appear on a dark background can be displayed in a lighter text color, making them easier to read. This feature must be enabled by your administrator. For more information about data labels on a darkbackground, see xref:chart-data-labels-on-darkbackground.adoc[].
+
This feature is in beta and off by default. To enable it, contact {support-url}.

Apply multiple changes at once  [.badge.badge-beta-relnotes]#Beta#::
Every time when a user drag and drop the fields in the chart configuration panel, the chart reloads. When a user has multiple fields, they need to rearrange a lot of them to reach a desired chart. Now users can make multiple configurations, and apply them rather than waiting in between each configuration. For more information, see xref:chart-multiple-chart-changes.adoc[].
+
This feature is in beta and off by default. To enable it, contact {support-url}.
