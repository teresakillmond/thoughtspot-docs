...
:linkattrs:
:experimental:
:description: You can enable Single Sign On (SSO) with your embedded version of ThoughtSpot.
...
Place the JS API library in the `<head>` section of the HTML on your webpage.
...
Your webpage needs to authenticate by calling `window.thoughtspot.initialize` and waiting for the `onInitializationCallback` to be called before embedding any ThoughtSpot visualizations or making any ThoughtSpot REST API calls.

The JS API call `window.thoughtspot.initialize` can cause the entire webpage to be re-directed to your Identity Provider (IDP).
...
The `onAuthExpiration` is only available if you have at least one ThoughtSpot visualization iframe in your webpage.
...
. Include the library file in your webpage's `<head>` section:
+
[source,html]
----
<head>
