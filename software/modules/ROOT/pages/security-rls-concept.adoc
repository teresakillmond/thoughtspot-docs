...
:linkattrs:
:page-partial:
:description: Use rule-based row-level security (RLS) to restrict a group's access to data.
:page-aliases: /admin/data-security/row-level-security.adoc

include::partial$liveboards-announcement.adoc[]

Use rule-based row-level security (RLS) to restrict a group or user's access to data.

Users see only accessible row data. Row-level security works at the group and user level and is configured on tables.
...
[#worksheets-queries-rls]
== Worksheet queries and RLS

You can't set RLS rules on worksheets, only on tables.
...
By default, ThoughtSpot implements a strict application of RLS rules, which takes into account RLS rules from all the tables underlying the Worksheet.
...
* Row-level security doesn't apply to them.
...
If the rule evaluates to `FALSE` for the user, then the user can't view the data and instead they see the message `No data to display`.
...
image::rls-example0.png[Table with date, vendor, and amount columns]
...
So, err on the side of simple rules rather than complex rules with a lot of logic. For example, performance is better with strings that exactly match the group name, instead of using complex string manipulation. Using `concat` doesn't usually impact performance.

* RLS rules don't support use of aggregate functions.
