...
After you complete the <<prereqs,prerequisites>>, you are ready to install ThoughtSpot clusters.
...
This additional service configuration is only required if you defined the xref:rhel-install-online.adoc#remote_user_management[remote_user_management] parameter in the Ansible playbook and used your LDAP or Active Directory service account for installation.
...
If you did _not_ define the xref:rhel-install-online.adoc#remote_user_management[remote_user_management] parameter, and used a local user for installation (the default), skip the following configuration. Move on to <<finalize,Finalize installation>>.
