= Selective Liveboard filters
:last_updated: 12/4/2020
:experimental:
:linkattrs:
:page-partial:
:page-aliases: /complex-search/selective-filters.adoc, pinboard-filters-selective.adoc

include::partial$liveboards-announcement.adoc[]

include::partial$liveboard-experience-new-announcement.adoc[]

Learn how to specify which Liveboard visualizations a Liveboard filter should apply to.

When you create or view a Liveboard, you may want to filter the visualizations on the Liveboard, to only see data that is relevant to your role, for example.
However, you may not want a filter to apply to every visualization in the Liveboard.
For example, the head of sales in North America may want to filter most visualizations to only contain data about sales in North America, but may want to compare her sales to an unfiltered _global quarterly sales_ metric.

You can apply Liveboard filters to specific visualizations from the Filters modal.

To learn more about Liveboard filters, refer to xref:liveboard-filters.adoc[Liveboard filters] and xref:liveboard-filters-linked.adoc[Linked Liveboard filters].

== Configure selective filters

. Click the *More* menu icon image:icon-more-10px.png[more options menu icon] for the Liveboard, and select *Edit*.

. In the top menu bar, select *Add filter*. If you already created the filter you would like to link, click on the filter in the filters bar below the Liveboard name, and proceed to step 6.
. In the *Configure filters* modal, choose the filter you would like to update from the side menu.
. In the populated columns menu, click the *Add filter* icon next to the column you would like to selectively filter on.
. Specify values to include or exclude, if you did not already specify them.
. Select *Applicable to* from the filters modal side menu.
You can select the *All* option, or only choose certain visualizations.
You can also search for a visualization.
+
image::filter-applicable-to-new-experience.png[Specify filtered visualizations]
. Select *Apply*.
ThoughtSpot applies the filter to the visualization(s) you specified.
