...
Performing a secondary level of aggregation across disjoint levels of detail may give nonsensical results. For example, aggregating to the activity classification level and then “aggregating up” to the account level doesn’t really make sense.
...
----
+
aggregates (sums) hours to the {account, activity classification} level of detail. This is comparable to what is shown in <<example-2-1-2,Table 2.1.2>> above.
+
...
----
+
averages the values associated with each account.
Snowflake: (5 + 2)/2 = 3.5
+
Redshift: (1 + 5)/2 = 3
+
...
In Example 3.1, our search bar contained account and activity date. Here we’ll add activity classification, so we are now computing Hours ignoring activity date at the {Account, Activity Classification} level of detail.
...
|===
+
In the table above, note that the three rows for Snowflake where the Activity Classification is “call” have the Hours value set to 1.67, while the line for Snowflake where the activity classification is “meeting” has Hours set to 2.
