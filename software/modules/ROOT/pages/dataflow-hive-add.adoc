...
:experimental:
:linkattrs:
:page-aliases: /data-integrate/dataflow/dataflow-hive-add.adoc
:description: You can add a connection to a Hive database using ThoughtSpot DataFlow.
...
Only when using Hiveserver2 HA.
 ...
 xref:dataflow-hive-reference.adoc#dataflow-hive-conn-port[Port]::
  Specify the port.
...
Define as `dfs.namenode.rpc-address.dfs.nameservices.name node ID 2`.
 ...
 xref:dataflow-hive-reference.adoc#dataflow-hive-conn-default-realm[Default realm]:: A Kerberos realm is the domain over which a Kerberos authentication server has the authority to authenticate a user, host or service (Configured from Kerberos configuration-/etc/krb5.conf).
...
For Hadoop Extract only.
 ...
 xref:dataflow-hive-reference.adoc#dataflow-hive-conn-jdbc-options[JDBC options]:: Specify the options associated with the JDBC URL.
