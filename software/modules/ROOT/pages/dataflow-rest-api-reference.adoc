...
:description: Learn about the fields used to create a REST API connection with ThoughtSpot DataFlow.
...
[#dataflow-rest-api-conn-response-format]
Response format:: Specifies the response format of the file. Mandatory field.
Example:;; JSON
Valid Values:;; JSON, LDJSON
...
REST API parameters:: When adding REST API parameters, select *Add*, and then specify the `Parameter name`, `Value`, and if the parameter is a `Header`. Optional field.
