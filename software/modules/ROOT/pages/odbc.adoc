...
Strictness = `true`, Permissiveness = `true`::
  Data types are inferred and automatically converted. ThoughtSpot returns an error in cases where the data conversion isn't possible. Data load fails in its entirety if any data contains mismatches. You must correct the problem in the source data and try the load again.
...
Strictness = `false`, Permissiveness = `true`::
  Data types are inferred and automatically converted. No error is thrown even if source and target data types don’t match. Data load continues even when the source and target data types don’t match. This means your data load may contain data types that you don't intend or that aren't helpful. You are responsible for checking and validating the data in this case.

Strictness = `false`, Permissiveness = `false`::
  No data types are inferred and conversion doesn't check for matches. This is the most permissive configuration.
...
NOTE: If a conversion isn't possible, ThoughtSpot returns an error.
The ETL tool must add a data transformation step if the source column data type doesn't exactly match the target's ThoughtSpot column data type.
The driver doesn't do any implicit conversions.
