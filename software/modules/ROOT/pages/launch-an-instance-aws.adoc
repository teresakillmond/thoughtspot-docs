...
The ThoughtSpot AMI has specific applications on a base image.
...
Starting with version 6.0.4, you can also deploy ThoughtSpot on AWS using Red Hat Enterprise Linux (RHEL), allowing you to run ThoughtSpot on an RHEL image that your organization manages internally.
...
| &#10063;
| Networking information: Download and fill out the ThoughtSpot xref:site-survey.pdf[site survey] to have a quick reference point.
...
| &#10063;
| A ThoughtSpot AMI.
For details, see xref:ami[Choose VM instances].

| &#10063;
| AWS security groups.
For required open ports, see xref:firewall-ports.adoc[Network ports].

| &#10063;
| AWS VM instances.
For instance type recommendations, see xref:configuration-options-aws.adoc#ts-aws-instance-types[ThoughtSpot AWS instance types].
...
. Choose the appropriate EC2 instance type: See xref:configuration-options-aws.adoc#ts-aws-instance-types[ThoughtSpot AWS instance types] for help choosing the correct instance type for your cluster.
...
Refer to xref:configuration-options-aws.adoc#ts-aws-instance-types[ThoughtSpot AWS instance types] for recommended nodes for a given data size.
...
. Click *Select*.
Ensure that you select the ThoughtSpot AMI listed xref:ami[above], which you entered earlier in this process.
+
...
(See xref:configuration-options-aws.adoc#ts-aws-instance-types[ThoughtSpot AWS instance types].)
...
For specific storage requirements, refer to xref:configuration-options-aws.adoc#ts-aws-instance-types[ThoughtSpot AWS instance types].
+
...
+
TIP: *Security setting for ThoughtSpot*+++<br>+++
 * The VMs need intragroup security, i.e.
every VM in a cluster must be accessible from one another.
For easier configuration, ThoughtSpot recommends that you enable full access between VMs in a cluster.+++<br>+++
...
+
Refer to xref:firewall-ports.adoc[Network ports].
