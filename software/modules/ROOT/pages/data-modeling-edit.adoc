...
The model file must be saved as a `.tsv` file, and it must be UTF-8 encoded. You may need to manually add the `.tsv` extension, if it is not an option.
...
. Change the file extension to `.tsv`. This ensures that your editor (Excel, vi/vim, and so on) knows the file contains tab-separated values.
