...
+
If you receive an error message upon uploading the file, check that the file is in TSV format, and that it doesn't include any multi-byte characters (for example, Japanese or other multi-byte  language characters).
If it does, you must download the file again and  make your edits using vi or vim.
+
You can remove all the rows you haven't changed from the model file before uploading it.
