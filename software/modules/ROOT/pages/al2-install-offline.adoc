...
| &#10063; | <<install-thoughtspot,5. Install ThoughtSpot>>
|===
...
After the Ansible Playbook finishes, run the `prepare_disks` script on every node. You *must* run this script with elevated privileges, as either a sudo or root user. Specify the data drives by adding the full device path for all data drives, such as `/dev/sdc`, after the script name. Separate data drives with a space.

Run the `prepare_disks` script, as either a sudo or root user:
