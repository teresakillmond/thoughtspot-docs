...
You can add multiple schedules with different configurations for a single Pinboard. However, each job is limited to one Pinboard schedule. To add a schedule, you must either be an administrator, or have the *Schedule pinboard* privilege, and at least view-only access to the pinboard.
