...
tscli backup create [-h] [--mode {full,light,dataless}] [--type {full,incremental}] [--base BASE] [--storage_type {local,nas,s3a,gcs}] [--remote] [--no-orion-master] [--bucket_name BUCKET_NAME] [--staging_dir STAGING_DIR] name out::
...
[source,tscli]
----
tscli cassandra [-h] {backup,restore,tablestats,truncatehints}
----
...
tscli cassandra truncatehints::
Runs `truncatehints` on all nodes. This can resolve issues where the cassandra check fails because of old hints being present.
...
--change_default_idp;; If you configured multiple IDPs, changes your default IDP. When users log into ThoughtSpot from the ThoughtSpot URL, the login screen takes them to the default IDP to log in. To use other, non-default IDPs that you configured with the `--multi` flag, users must log into ThoughtSpot from the other IDP's login page.
+
The default is `False`.
...
[source,tscli]
----
tscli support [-h] {bundle,notification-banner,remove-maintenance,remove-notification-banner,restart-remote,rm-admin-email,rm-admin-phone,rm-feedback-email,schedule-maintenance,set-admin-email,set-admin-phone,set-debug-ui-password,set-feedback-email,set-remote,show-admin-email,show-admin-phone,show-feedback-email,show-remote,start-remote,stop-remote}
----
...
tscli support notification-banner::
Enables the display of a notification banner. This command has the following optional subcommands:

--colour COLOUR;; Background color of the banner. The default is yellow.
--message MESSAGE;; The content of the notification banner.

tscli support remove-maintenance::
  Removes a scheduled maintenance.

tscli support remove-notification-banner::
