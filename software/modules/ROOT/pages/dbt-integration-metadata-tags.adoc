...
Default values;; If the column type is `measure`, the default is `yes`. If the column type is `attribute`, the default is `no`, and you can't change the value to `yes`.
Sample;;
+
----
- name: QUANTITYPURCHASED
