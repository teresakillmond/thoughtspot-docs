...
:linkattrs:
:experimental:
:description: Configure your nodes before you can install your cluster(s) on the SMC appliance.
...
If the network configuration wasn't pre-set, then this step must be done as part of the installation process.
...
If your status is not READY, continue with the installation process.
...
If your `/install` subdirectory doesn't exist, you may have to use the `/home/admin` directory.
