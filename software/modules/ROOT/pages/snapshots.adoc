...
:experimental:
:linkattrs:
:description:  In this section, we describe how to work with the default snapshot configuration that is enabled on every cluster, and how to make manual snapshots.
...
To restore from a snapshot, contact {support-url}.
...
Do not change the default policy, unless it is recommended by {support-url}.
