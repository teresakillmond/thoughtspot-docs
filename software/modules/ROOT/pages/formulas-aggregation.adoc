...
* xref:about-pinned-measures.adoc[Group aggregation functions] apply a specific aggregate to a value, and group the results by an attribute in the data.
* xref:about-cumulative-formulas.adoc[Cumulative functions] measure from the start of your data to the current point. We often use the for time-based data.
* xref:about-moving-formulas.adoc[Moving formulas] measure within a window (usually time-based) that you define.
...
. xref:create-ws-filter.adoc#_use_aggregate_formulas_as_worksheet_filters[Add the formula to Worksheet as a filter], and specify conditions in which it should apply.
