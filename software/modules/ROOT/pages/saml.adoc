...
----
+
To change your default IDP, add `--change_default_idp` to the end of the command. When users log in to ThoughtSpot from the ThoughtSpot URL, the login screen will take them to the default IDP. To use other, non-default IDPs that you configured with the `--multi` flag, users must log in to ThoughtSpot from the other IDP's login page.
+
[source]
----
