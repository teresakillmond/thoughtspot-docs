= Other group aggregation formulas
:last-updated: 11/01/2021
:linkattrs:
:experimental:

In this article, we will consider examples of aggregation formulas other than group_aggregate(). To test for yourself, use the data set provided in <<prerequisites,the prerequisite>>.
...
NOTE: * Outer aggregation is required for the group_* functions in exactly the same manner that it is required for group_aggregate(). +
* The only way to specify that the level of detail should include the search level of detail is to use group_aggregate() with the query_groups() functionality. There is no way to use query_groups() with the  group_* functions. +
...
The following formula will calculate this correctly, regardless of the level of detail specified in the search:
...
If your search includes _account_, this will aggregate correctly to the account level of detail by averaging the values calculated by the inner aggregation, which are at the query_groups() + {activity classification} level of detail.  The inner aggregation will create a “virtual table” that calculates that average like this:
...
If the search contains _account_, the final table will look exactly like that shown above.
...
|===
+
If you do not include `account` in the search bar, it will aggregate up to the table level, averaging the four values.

2. Search for: `average hours per activity classification`
+
[options=”header”]
|===
| average hours per activity classification
...
The inner aggregation will aggregate to the activity classification level of detail. In this case, the inner aggregation will create a virtual table that looks like this:
