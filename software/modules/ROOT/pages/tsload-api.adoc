...
:last_updated: 03/22/2022
:linkattrs:
:experimental:
:description: The tsload connector APIs enable you to load data into ThoughtSpot.
...
After this completes, use *Load* to start the actual data load.
...
. If there are fewer columns in the input file, the system fills the missing columns using `null` values.
+
...
> **Related information**
>
> * xref:tsload-connector.adoc[Use the tsload connector to load data]
