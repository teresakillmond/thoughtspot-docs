...
In this article, we will walk through examples of group aggregation using filters. To test for yourself, use the data set provided in <<prerequisites,the prerequisite>>.
...
In this example, we’ll apply a filter in the search bar that filters for `activity classification = “call”`.

1. First, let’s look at what our table looks like with no filter. This is created by the following search:
...
.. In the formula pop-up window, name your formula. Here, we have named the formula *hours per account*.
...
4. Finally, add that formula to our search bar.

.. Search for: `account` `activity classification`  `hours`  `hours per account`   `activity class = call`  `sort by account`.
...
8. Select *Formulas* from the left sidebar and click the *edit icon* next to "hours per account".
9. In the formula window, enter the following formula and click *Save*:
[source]
----
average(
  group_aggregate(
...
For example, create the following formula if you want your formula to explicitly filter for `activity class = “call”`, regardless of what is in the search bar:
...
Search for: `account`   `activity classification`  `hours`   `hours per account`   `sort by account`   `sort by activity classification`
...
You can put multiple filters in the filters argument of the group_aggregate function. For example, use the following formula if you want to filter on both activity class and activity date:
...
In the above example, the activity classification and activity date filters will be added in an `and` condition together.
...
You can also use AND, OR, and parens, to flexibly combine filters:
