...
If there are no rows matching the filter criteria, the condition returns a `0` (zero). When you use a filter rather than a filter function, ThoughtSpot does not include zero values. Using a filter function like `sum_if` or `count_if` is the best way to get zero values on your chart or table.
