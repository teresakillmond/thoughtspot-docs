...
When you xref:liveboard-schedule.adoc[schedule a pinboard job], you can add a gating condition that triggers the pinboard email. If the condition evaluates to `true` at the scheduled time, ThoughtSpot emails the pinboard to the specified recipients. If it evaluates to `false`, ThoughtSpot does not send the pinboard.
...
After you create your view and pinboard, follow the directions in xref:liveboard-schedule.adoc[] to create the pinboard schedule.
