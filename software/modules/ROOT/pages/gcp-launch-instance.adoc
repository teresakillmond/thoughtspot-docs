...
:linkattrs:
:experimental:
:description: Set up your GCP virtual machines.
...
. Select *CREATE BUCKET* on the top menu bar.
. Enter a name for your bucket, and select *CONTINUE*.
. For location type, select *Region*.
. Use the Location dropdown menu to select the region where you are going to set up your instance.
. Select *CONTINUE*.
...
. Select the *+* icon from the top menu bar to create an instance.
...
.. Scroll down to the *Boot disk* section and select *Change*.
+
// image::gcp-4-change-boot-disk.png[Change boot disk]
.. Select *Custom Images* from the options under *Boot disk*.
...
+
Release 8.4.0.sw [LA] and 8.4.1.sw:: thoughtspot-image-20210402-71f6832a800-prod

ThoughtSpot updates the base images with patches and enhancements. If more than one image is available, select the latest one by looking at the dates of creation. Each image works; however, we recommend using the latest image because it typically contains the most recent security and maintenance patches. contact {support-url} if you are unsure which image to use.
...
.. Select the *Disks* tab, and click *Add new disk*.
+
...
. (For use with GCS only) After you select *Set access for each API*, scroll down to the *Storage* dropdown menu in the Identity and API access section.
...
+
Network:: Add an existing VPC network, or create a new one by selecting *VPC network* from the main menu.
...
Before you can install your ThoughtSpot cluster, an administrator must sign in to each VM through SSH as user "admin", and complete the following preparation steps:
