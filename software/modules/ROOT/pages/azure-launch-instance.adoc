...
To install ThoughtSpot using RHEL, refer to the xref:customer-os.adoc[Deploying ThoughtSpot on your own OS guide] after you launch your virtual machines.
...
+
In a browser, go to https://portal.azure.com/#home[Azure home^], and sign in to your Azure account.
...
+
NOTE: This user is necessary for Azure VM creation, but does not exist in ThoughtSpot. You cannot sign in to ThoughtSpot, or `ssh` into the command line, with this user.
...
_Prerequisite_: To sign in to the VM, you need the private key that is available in the image.
...
+
Sign in as the admin user, using the private key that your ThoughtSpot contact sent you.
+
[source,console]
----
...
If you are deploying ThoughtSpot on ThoughtSpot-managed CentOS, your result may look something like the following listing. If you are deploying ThoughtSpot on RHEL, OEL, or AL2, your result should display the partitioning specified in xref:customer-os-prerequisites.adoc#partition-hosts[Prerequisites for deploying ThoughtSpot on your own OS: Partition the hosts].
