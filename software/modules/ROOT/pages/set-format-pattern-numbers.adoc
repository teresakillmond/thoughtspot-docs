...
Learn how to set key formats for column values.
...
You can change the date format used to display a column's values xref:model-data-in-UI.adoc[for a single table or Worksheet] or, by editing the data model, for xref:edit-model-file.adoc[the entire ThoughtSpot instance]. To edit the data model file, you must have administrative privileges. To change the date format, follow these steps:

. Decide if the change is for a table or Worksheet, or for the entire instance.
...
NOTE: You can see this custom date formatting when you use the `date detailed` keyword. Other date buckets, such as `monthly` or `weekly`, use the standard format. For daily datess, ThoughtSpot uses each other user's locale to determine whether to display `mm/dd/yyyy` or `dd/mm/yyyy`.
...
You can change the currency format used to display a column's values xref:model-data-in-UI.adoc[for a single table].
