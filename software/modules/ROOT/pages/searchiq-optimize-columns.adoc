...
See the xref:stop-words.html[Stop Words] reference for the full list of stop words.
...
+
See xref:set-entity-category.html[Set Entity Categories].
...
See xref:change-visibility-synonym.html[Add common synonyms] for more information.
+
...
We recommend excluding a column when it is descriptive in nature, contains a large amount of text, has xref:stop-words.html[stop words], or for a variety of similar reason.
See xref:change-index.html[Turn off indexing].
+
