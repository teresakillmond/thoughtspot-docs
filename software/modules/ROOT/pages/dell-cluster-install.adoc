...
:description: Install your ThoughtSpot cluster(s) on your Dell appliance.

Install your ThoughtSpot cluster(s) on your Dell appliance. If you are deploying ThoughtSpot using xref:customer-os.adoc[RHEL, OEL, or Amazon Linux 2], refer to xref:customer-os-install.adoc[] instead of this article.
...
If you don't have a link to download the release bundle, open a support ticket at xref:support-contact.adoc[ThoughtSpot Support] to access the release bundle.
...
`hostname`::
  is your network hostname; ask your network administrator if you don't know your hostname.
`file-name`::
...
Sign in to any node to check the current cluster status, using the command `tscli cluster status`.
