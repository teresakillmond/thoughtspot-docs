...
:linkattrs:
:experimental:
:page-aliases: admin/setup/LDAP-config-AD.adoc, LDAP-config-AD.adoc
...
NOTE: This article describes how to set up integration with LDAP using Active Directory.
...
To enable user authentication through OpenLDAP, xref:support-contact.adoc[contact ThoughtSpot Support].
...
* [ ] xref:url[1. URL]
* [ ] xref:domain-name[2. Domain name]
* [ ] xref:search-base[3. Search base]
* [ ] xref:ssl[4. SSL]
* [ ] xref:auto-add[5. Automatically add LDAP or AD users in ThoughtSpot]
* [ ] xref:ts-auth[6. Use ThoughtSpot internal authentication]
...
Authentication against multiple domains isn't supported.
...
This is useful in scenarios where some users aren't in AD and are created only in ThoughtSpot.

[#configure-ldap-tscli]
== Configure LDAP using tscli

You don't have to create a user called `tsadmin` on your LDAP server.
...
. If you are using SSL, xref:ldap-ssl.adoc[add the SSL certificate for AD].
