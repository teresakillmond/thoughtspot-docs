...
Alternatively, use ThoughtSpot’s built-in monitoring alert system. Specify an email address using the `tscli monitoring set config --email` command to route all ThoughtSpot-generated alerts to that email address. Optionally customize the email content. See xref:monitoring-setup.adoc[].
