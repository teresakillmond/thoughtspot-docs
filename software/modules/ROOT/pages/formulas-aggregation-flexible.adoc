...
ThoughtSpot provides flexible aggregation with the `group_*` function.
...
If the sub-query is at less detailed than the original query, ThoughtSpot adds the result column to the result of original query.
When the sub-query is at a finer detail level than the original query, ThoughtSpot re-aggregates to formula's results to match the groupings of the original query.
