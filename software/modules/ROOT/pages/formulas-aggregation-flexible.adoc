...
Beginning in the 9.0.0.sw release, you can specify a column in the third argument of a group_aggregate function in order to include all filters defined in the search that pertain to that column, and ignore unrelated filters. This enhancement allows the analyst to specify which fields to accept filters from, improving readability of formulas, and reducing operational change and related errors. The group aggregation filters enhancement feature is in beta and off by default. To enable this feature, contact ThoughtSpot Support.

For example, say that you would like to determine how many stores a certain product was sold in during a given time period, expressed as a ratio of total available stores. In this formula, you would need to accept the filters associated with transaction date (for example, this year, last six months, august), and ignore filters from other fields.

Before 9.0.0.sw, this formula would look like: `group_aggregate(count(storid),query_groups(),query_filters-{field1,field2,fieldN})`, where users would have to use `query_filters - {}` to manually remove every field in the search bar that did not relate to transaction date. When adding a new unrelated filter to the search bar, users would then have to edit all existing formulas to ignore the filter.
