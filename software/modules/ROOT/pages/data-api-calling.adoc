...
== Specify the Liveboard or visualization example

id:: For a Liveboard, you can append the ID of your Liveboard as a parameter, like this example:
+
[source,html]
----
...
vizid:: To retrieve data from a specific visualization within a Liveboard, you would append the ID number of the visualization using the vizid parameter:
+
...
When you parse the returned JSON data you can see that there is one object for every viz on the Liveboard.
The objects are named according to the corresponding vizid.

image::parsed_json_data.png[List of objects in the JSON data]

If you make a call to a specific viz on a Liveboard, it returns only one object.
...
image::columnnames_and_data_arrays.png[JSON columnNames array, expanded]
