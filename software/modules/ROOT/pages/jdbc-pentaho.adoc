...
. Sign in to the local machine where you have already installed the Pentaho Data Integration (PDI) client.
. Click xref:downloads.adoc[*here*] to download the JDBC driver.
. Select *JDBC Driver* to download the file `thoughtspot_jdbc<version>.jar`.
...
. Select *New* to create a new transformation.
+
image::new_transformation_pentaho.png[View > Transformations > New]

. Select *Input* under the *Design* tab to expand it.
...
. Name the *Step*.
. Select *Browse* next to the *Filename* field and provide the file you want to read from.
. Select *OK*.
+
image::browse_filename_pentaho.png[Add your CSV file and select OK]

. In the CSV Input dialog, select *Get Fields*.
...
Select *OK* to start the transformation in preview.

. Examine the preview data, then select *Close*.
+
...
. Select *OK* in the CSV Input dialog to confirm your CSV input settings.

=== Define the Output

. Select menu:Design[Output].
...
. Select *Test* to test your database connection.
. If you are able to make a successful connection to the ThoughtSpot Simba Server, select *OK*.
+
image::database_connection_test_pentaho.png[After you test, if the connection worked, select OK.]

. Select *OK* in the Database Connection dialog to create the new connection.

=== Import data

. In the *Table output* dialog, select the connection you just created.
. Select *Browse* next to the *Target schema* field and select your *Target schema*.
...
. In the *Simple SQL editor* dialog, select *Execute*.
+
...
. Select the *Play* button at the top of the *Transformation* window to execute the transformation.
+
image::execute_the_transformation_pentaho.png[Execute a transformation]

. Select *Launch* in the *Execute a transformation* dialog.
+
