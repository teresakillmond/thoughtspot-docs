...
ThoughtSpot's ODBC connection relies on the link:https://www.simba.com/products/SEN/doc/Client-Server_user_guide/content/clientserver/configuringsimbaclientodbc/simbaclientodbcunix.htm[SimbaEngine X SDK,window="_blank"] to connect through ODBC or JDBC to ThoughtSpot's remote data stores.
...
Make sure your Windows workstation is equipped with a tool link:https://www.putty.org/[such as Putty,window="_blank"] for making `ssh` connections to your ThoughtSpot server.
...
If it isn't installed, make sure you link:https://www.microsoft.com/en-us/download/details.aspx?id=40784[download and install it,window="_blank"] before continuing.
