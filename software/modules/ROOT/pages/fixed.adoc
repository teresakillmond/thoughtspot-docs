...
+
--
* Remote code execution Java vulnerability in the H2 database console (CVE-2021-42392).
* Client-state upgrader drops numeric formatting of new answers.
* The join type cannot be changed from INNER to RIGHT OUTER in a worksheet.
* Connection is refused when checking the status of a load.
* Runtime filters are dropped when a user connects through SSO.
* Dataflow syncs fail with a broken pipe issue because TSLoad as service times out.
* TML for pinboards contains incorrect conditional formatting.
* Search is slow.
* Authentication issues occur while embedding pinboards from an external web application.
* Oracle ADW tables cannot be queried.
* SQL generation results in a wrong join direction.
* More than one root can be seen in a query.
* Tomcat library version updated from 8.5.61 to 8.5.70.
* Users cannot load pinboards or see them in the pinboards page.
* Most pinboards and answers cannot be opened.
* Formats are lost when doing a TML import or export.
* Duplicate column names are not allowed.
* Table path prefix is dropped when a formula with a similar name exists.
--
7.1.1::
+
--
