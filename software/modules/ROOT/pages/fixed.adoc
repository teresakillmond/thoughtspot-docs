...
[#releases-8-8-x]
== 8.8.1.sw releases fixed issues
...
|SCAL-130542
|After upgrade to 8.7.0.cl-111, some formulas using a mix of `group_*`  and other functions incorrectly default to `MIN` instead of `SUM` aggregation.

|SCAL-129737
|Headline values are less precise because they are rounded off to the nearest decimal value. This only happens when the dataType is `DOUBLE`.

Example: Showing 46 instead of 45.67.

|SCAL-125860
|Large data sets with multiple values for a pivot table display as "undefined". Calculation of such data sets uses a lot of CPU cycles and causes a delay in render times for pivot tables. Optimized on that calculation.
|===
