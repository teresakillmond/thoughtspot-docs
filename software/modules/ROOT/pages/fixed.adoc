...
* Connection is refused when checking the status of load.
* Runtime filters are dropped when user connects through SSO.
...
* Search is slow.
* Authentication issues while embedding pinboards from an external web application.
...
* User cannot load pinboards or see them in the pinboards page.
* Most pinboards/answers cannot be opened.
