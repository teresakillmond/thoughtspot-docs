...
|SCAL-147691
|Cannot search by names when sharing an object in release 8.4.1.sw.

|SCAL-142085
|Pivot table with a group aggregate formula displays wrong numbers.

|SCAL-138127
|Unable to update username/password of a connection.

|SCAL-134963
|Visualization built on a view with measure filters results in the following error: "The visualization data could not be retrieved. Failed to generate worksheet".
|===
...
|SCAL-134677
|Using Windows, saving a formula no longer adds extra blank lines in the formula.

|SCAL-132991
|ThoughtSpot Everywhere customers see a red error banner at the top of search when doing their first search.

|SCAL-132347
|REST API v2: gettoken returns a 404 error.

|SCAL-129920
|Custom actions do not work for context actions in Liveboard v2.

|SCAL-129691
|"UniqueCount is not supported for the current query” error displayed on pinned headline tiles in a Liveboard.

|SCAL-129104
|The disabled "Powered by ThoughtSpot" footer text no longer shows in an embedded Liveboard v2 mode.

|SCAL-127526
|"Couldn't complete your request" error when user try to load more rows in table view mode.
|===
