...
Creating groups and assigning users to them makes privilege management easier. Before a user can sign in and use ThoughtSpot, you must create a username, a password, and a membership in one or more groups for them.
...
You can't delete the *All* group or remove members from it. Only users with either the *Can administer ThoughtSpot* or *Can share with all users* privilege can share an object with the *All* group.
...
Permissions to see and edit tables, worksheets, and Liveboards are set when you share them with users and groups, as described in xref:data-security.adoc[Data security].
