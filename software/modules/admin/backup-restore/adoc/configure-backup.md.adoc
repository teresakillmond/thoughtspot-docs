...
You can backup to a local file system, link:{{ site.baseurl }}/admin/setup/NAS-mount.html#[mount a NAS (network attached storage) file system] to hold the backup, or link:{{ site.baseurl }}/appliance/aws/aws-backup-restore.html[back up to an S3 bucket].
...
{% include note.html content="To periodically back up your cluster to an S3 bucket, specify `S3` for the `storage_type`, and include the `enabled` and `bucket_name` parameters." %}
