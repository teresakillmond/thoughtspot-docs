...
You can change the date format used to display a column's values xref:model-data-in-UI.adoc[for a single table] or, by editing the data model, for link:{{ site.baseurl }}/admin/data-modeling/edit-model-file.html[the entire ThoughtSpot instance].
...
include::includes:partial$data-modeling-strings.adoc[]

To change the date format used to display a column's values xref:model-data-in-UI.adoc[for a single table] or, by editing the data model, for xref:edit-model-file.adoc[the entire ThoughtSpot instance].
