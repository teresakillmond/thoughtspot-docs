= Install the ThoughtSpot application on offline clusters that use RHEL
...
ts-new.yaml:: The Ansible Playbook for new installations. Note that this is called `ts-rhel-new.yaml` in ThoughtSpot release 6.1. It is `ts-new.yaml` in releases 6.1.1 and later.
ts-update.yaml:: The Ansible Playbook for updates. Note that this is called ts-rhel-update.yaml in ThoughtSpot release 6.1. It is `ts-update.yaml` in releases 6.1.1 and later.
ts.yaml:: Note that this is called `ts-rhel.yaml` in ThoughtSpot release 6.1. It is `ts-yaml` in releases 6.1.1 and later.
