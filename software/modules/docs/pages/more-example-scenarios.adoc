...
last_updated: 12/14/2020 summary: "It's not always easy to know when you need to use a view, but these are some common scenarios." sidebar: mydoc_sidebar permalink: /:collection/:path.html -- Here are four common examples of when you would want to use a view.
...
This example could also be solved in a single search by using link:{{ site.baseurl }}/advanced-search/formulas/conditional-sum.html#[conditional formulas] to determine the desired group (or cohort) of customers as defined by their buying behavior, and then doing the analysis on this group.
But the performance will be better if you do this using a view, and then link:{{ site.baseurl }}/admin/loading/materialized-views.html#[materialize the view].
