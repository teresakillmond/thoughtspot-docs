...
| *Access* | Native (JDBC)
| *Custom Connection URL* a| `jdbc:simba://SERVER_IP:12345;Database=DATABASE_or_SCHEMA_NAME`

The IP is a node in your ThoughtSpot cluster. The name or schema of the database you want to connect to. Use TQL to create a database name if needed. Ensure that there are no leading or trailing spaces.

|*Custom Driver Class Name*  a| `com.simba.client.core.jdbc4.JDBC4Driver`
