...
If you are using the xref:https://docs.aws.amazon.com/systems-manager/latest/userguide/ssm-agent.html[AWS SSM agent, window="_blank"] as an alternative to SSH, you must run the Ansible playbook and all commands on the __SSM console__. Ensure that your nodes have the right IAM roles for executing SSM scripts, and that they have access to your S3 bucket, if you are using S3.
...
* Ansible installs the required packages: YAML, Python, and R packages; see xref:amazon-linux-2/al2-packages.adoc[Packages installed with ThoughtSpot for Amazon Linux 2].
...
| &#10063; | xref:redirect-mirror[3. Redirect the mirror repository]
| &#10063; | xref:run-ansible[4. Run the Ansible Playbook]
