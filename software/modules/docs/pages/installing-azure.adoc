...
. *Change the admin password* Refer to xref:launch-an-instance-azure.adoc#prepare-for-startup[Prepare for starting up ThoughtSpot] to learn how to log into your VM with the private key, and change the admin password.
You must change the admin password on first login, or you will not be able to log back into your VMs.
. *Review required ports* Refer to xref:firewall-ports.adoc[Network ports] to view the required ports for successful operation of ThoughtSpot.
...
Log in using the new password you created for the _admin_ user in step two of xref:launch-an-instance-azure.adoc#prepare-for-startup[Prepare for starting up ThoughtSpot].
...
WARNING: _DNS server_ and _IP address_ values must be set in the Azure cloud backend.
...
xref:https://training.thoughtspot.com/[ThoughtSpot U,window=_blank]
