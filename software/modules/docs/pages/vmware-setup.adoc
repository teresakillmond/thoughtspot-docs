...
To install ThoughtSpot using RHEL, refer to the xref:rhel.adoc[RHEL deployment overview].
...
For production deployments, ThoughtSpot requires you to have three 2TB HDFS disks on HDD (3 x 2TB). Refer to xref:vmware-intro.adoc#supported-configurations[Supported configurations] for more information. For this use case, follow these same steps to create the additional, larger capacity disks.
...
. Save your changes.
. Repeat steps 1-5 to create more hard disks. ThoughtSpot requires 3 hard disks for a production environment.
. Power on the VM.
