...
. Use xref:about-nested-formulas.adoc[nested formulas] to calculate the percent increase by creating a parent formula: `percent increase = ( ( this week revenue - last week revenue) / last week revenue ) * 100`
...
Your ThoughtSpot administrator and ThoughtSpot Support can xref:set-custom-calendar.adoc[set up a fiscal calendar year] to start on any month.
