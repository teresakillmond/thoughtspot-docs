...
+
Refer to your welcome letter to find a link to download the release tarball. If you do not have a download link, open a support ticket with xref:contact.adoc[ThoughtSpot Support] to access the release tarball.
...
+
If you are using an S3 or GCS bucket for object storage, include the flag `--enable_cloud_storage s3a` or `--enable_cloud_storage gcs`. GCS is GCP's object storage, and s3a is AWS's object storage.
+
s3 example:
+
[source,tscli]
