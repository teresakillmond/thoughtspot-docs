...
:experimental:
:linkattrs:
:redirect_from: /complex-search/about-pivoting-a-table.html
...
If you right-click a row or column heading, the system displays a contextual menu, allowing you to *exclude* or *include* values, xref:drill-down.adoc[drill down], *show underlying data*, *SpotIQ analyze*, or *sort*.

image::charts-pivot-table-sort.png[Sort by a row heading]

image::charts-pivot-table-column.png[Sort by a column heading]

If you right-click a table cell, the system displays the contextual menu, so you can *exclude* or *include* values, xref:drill-down.adoc[drill down], *show underlying data*, or *SpotIQ analyze*.
