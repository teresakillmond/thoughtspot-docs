...
Use rule-based RLS to restrict a group's access to data.

Users see only accessible row data.  Row level security works at the group level and is configured on tables.
...
* To learn the procedure you follow for setting a rule, xref:set-rls.adoc[Set RLS rules]
...
* For information on bypassing rules on a worksheet, see xref:change-inclusion-rule.adoc[Change inclusion, join, or RLS for a worksheet].
