...
The unique key ThoughtSpot uses to identify IDP service. You may know this as the _Entity ID_.
...
The absolute path to the `idp-meta.xml` file is needed for one-time configuration. If your IDP needs an Assertion Consumer Service URL to create the metadata file, use `https://<hostname_or_IP>/callosum/v1/saml/SSO`.
...
Instead, you must xref:user-management.adoc#add-user[add users manually] or through xref:ldap-config-ad.adoc[Active Directory].
