...
To restore from a snapshot or backup, contact xref:appliance:contact.adoc[ThoughtSpot Support].

== Restoring a ThoughtSpot cloud deployment

For details on how to restore a ThoughtSpot cluster hosted in the cloud, see:

* xref:appliance:aws-backup-restore.adoc[Backup and Restore with AWS]
* xref:appliance:gcp-backup-restore.adoc[Backup and Restore with GCP]
