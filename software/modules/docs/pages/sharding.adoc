...
+++<dl>++++++<dlentry>++++++<dt>+++HASH+++</dt>+++
+++<dd>+++Determines the number of shards.
In this case, `96`.+++</dd>++++++</dlentry>+++
+++<dlentry>++++++<dt>+++KEY+++</dt>+++
+++<dd>+++Specifies how to assign data into the shards (shard key).
In this case, `customer_id`.+++</dd>++++++</dlentry>++++++</dl>+++
...
+
`PRIMARY KEY("saleid,vendorid")` on A +++<br>+++  `PRIMARY KEY("saleid,customerid")` on B
+
