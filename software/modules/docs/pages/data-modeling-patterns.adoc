...
These can only contain numbers, and alpha characters are not permitted.
...
[horizontal]
`\#,\##0.##`:: 12345.6789 -> 12,345.68
`\#,\##0.\###`:: 12345.6789 -> 12,345.679
`\#,##0.00000`:: 12345.6789 -> 12,345.68
`\#,##0`:: 12345.6789 -> 12,345
`\#,##0.00`:: 12345.6789 -> 12,345.68
`\#,\##0.##`:: 12345.6789 -> 12,345
`\#,##0.00`:: 12345.6789 -> 12,345.00
...
For example, if you are using ThoughtSpot in the US, the number formatting looks like this: `xxx,xxx.xx`, because the decimal separate in US is the `.` (period).
And in Europe, it looks like this: `xxx.xxx,xx`, because the European decimal separater is the `,` (comma).

[#date-format]
== Date formats

The *Format Pattern* option in the UI or in the model file lets you to specify how ThoughtSpot displays dates in tables and charts.
For example, you can display dates in a standard European or US formats, based on the data modeling setting *Format Pattern*.
These are some examples of formats you can use:

include::partial$data-modeling-strings.adoc[]

You can change the date format used to display a column's values xref:model-data-ui.adoc[for a single table or Worksheet] or, by editing the data model, for xref:data-modeling-edit.adoc[the entire ThoughtSpot instance]. To edit the data model file, you must have administrative privileges. To change the date format, follow these steps:
