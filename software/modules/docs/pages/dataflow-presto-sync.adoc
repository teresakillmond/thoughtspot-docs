= Sync data through a Presto connection
:last_updated: 7/7/2020
:experimental:
:linkattrs:
:redirect_from: /data-integrate/dataflow/dataflow-presto-sync.html



After using ThoughtSpot DataFlow to establish a connection to a Presto database, you can create automatic data updates, to seamlessly refresh your data.

include::partial$sync-for-databases.adoc[]

. Specify the sync properties for Presto:
+
// ![Enter connection details]({{ site.baseurl }}/images/dataflow-presto-sync.png "Enter connection details")
 ** xref:dataflow-presto-reference.adoc#dataflow-presto-sync-column-delimiter[Column delimiter] +
  Specifies the column delimiter character.
 ** xref:dataflow-presto-reference.adoc#dataflow-presto-sync-null-value[Null value] +
  Specifies the string literal that indicates the null value in the extracted data.
During the data load, the column value matching this string loads as null in the target.
 ** xref:dataflow-presto-reference.adoc#dataflow-presto-sync-enclosing-character[Enclosing character] +
  Specify if the text columns in the source data needs to be enclosed in quotes.
 ** xref:dataflow-presto-reference.adoc#dataflow-presto-sync-escape-character[Escape character] +
  Specify the escape character if using a text qualifier in the source data.
 ** xref:dataflow-presto-reference.adoc#dataflow-presto-sync-ts-load-options[TS load options] +
  Specify the parameters passed with the `tsload` command, in addition to the commands already included by the application.

+
See xref:dataflow-presto-reference.adoc#sync-properties[Sync properties] for details, defaults, and examples.
. Save your work by clicking *Save*. +
 Alternatively, click *Save and sync now* to save your work and sync data at the same time.
