...
+ To ensure that the query on such tables honors the partition condition, you must create a worksheet filter in ThoughtSpot.+++</dlentry>++++++</dlentry>++++++<dlentry>+++Azure Synapse::::  Azure Synapse supports up to 10 `IF THEN ELSE` statements in a single query.+++</dlentry>++++++<dlentry>+++Azure Synapse does not support foreign keys, so no PK-FK joins can be defined in Synapse.+++</dlentry>+++
