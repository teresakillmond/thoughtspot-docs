...
* [ ] xref:url[URL]
* [ ] xref:domain-name[Domain name]
* [ ] xref:search-base[Search base]
* [ ] xref:ssl[SSL]
* [ ] xref:auto-add[Automatically add LDAP or AD users in ThoughtSpot]
* [ ] xref:ts-auth[Use ThoughtSpot internal authentication]
...
Next, xref:sync-users-and-groups-from-ldap.adoc[sync users and groups from LDAP through Active Directory].
