...
If you do not have a link to download the release bundle, open a support ticket at https://community.thoughtspot.com/customers/s/contactsupport[ThoughtSpot Support,window="_blank"] to access the release bundle.
...
+
If you prefer a method other than the `scp` command, use `tscli fileserver download-release` to download the release bundle.
...
. In the xref:install-cluster-dell.adoc#ts-login[ThoughtSpot sign-in window], enter admin credentials, and click *Sign in*.
