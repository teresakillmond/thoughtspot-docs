...
For example, you can xref:query-on-query.adoc[create a View] to use the Answer as a data source, xref:replay-search.adoc[replay the search] required to create the Answer, download the Answer, or export an Answer as a xref:tml.adoc[TML], or ThoughtSpot Modeling Language, file, optionally modify it, and xref:scriptability.adoc[migrate it to another cluster], or re-upload it to the same cluster.
