...
see xref:admin:add-user.adoc#add-user[Create a user].
xref:appliance:contact.adoc[Contact ThoughtSpot Support] to add new email domains to your list of allowed domains.
+
...
See xref:admin:DAP-config-AD.adoc[Configure LDAP for Active Directory].
...
See steps for xref:admin:add-group.adoc#add-group[creating a user group] or xref:admin:add-group.adoc#edit-group[editing a user group].
. Create a new user and assign them to the group you are using for onboarding.
See steps for xref:admin:add-user.adoc#add-user[creating a new user].
You can also add existing users to the group;
see
+
In addition to creating a new user directly in ThoughtSpot, you can add users in bulk through xref:admin:active-directory-based-access.adoc[Active Directory].
