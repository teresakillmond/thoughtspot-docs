...
+++<ul>++++++<li>+++Are exempt from row-level security (RLS) rules.+++</li>+++
    ...
    +++<tr>++++++<td>++++++<strong>+++Has Developer privilege+++</strong>++++++</td>+++
      +++<td>+++Can access and use the xref:spotdev-portal.adoc[Developer Portal] to explore the ThoughtSpot APIs and developer tools, and build web applications with ThoughtSpot content.+++</td>++++++</tr>+++
    +++<tr id="read-only">++++++<td>++++++<strong>+++Cannot create or update Liveboards+++</strong>++++++</td>+++
      ...
      +++<p>+++This privilege is designed to support embedded implementations, and is not available by default. xref:support-contact.adoc[Contact ThoughtSpot Support] to enable it.+++</p>+++
      +++<p>+++See xref:liveboard-granular-permission.adoc[Granular access to Liveboards] for a deeper discussion of this privilege, and an implementation example.+++</p>++++++</td>++++++</tr>++++++</tbody>++++++</table>+++
