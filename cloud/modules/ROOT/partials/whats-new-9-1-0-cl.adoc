...
[discrete]
=== Free Trial - Show row limits [.badge.badge-beta]#Beta#
endif::[]
ifdef::pendo-links[]
[#9-1-0-cl-free-trial-row-limits]
[discrete]
=== Free Trial - Show row limits [.badge.badge-beta-whats-new]#Beta#
endif::[]
...
change analysis
regardless of user privileges. Only users with the *Has SpotIQ privilege* can run custom analysis.
...
You can now pull data from a ThoughtSpot instance into Google Sheets using the *ThoughtSpot for connected sheets* extension. With this extension, you connect to your ThoughtSpot instance, then select your data using the search bar or the data panel, just like in ThoughtSpot. After your data is in Google Sheets, you can use the available tools to further analyze your data. Coming soon in the Google Workspace Marketplace.

*Note*: To use *ThoughtSpot for connected sheets*, you must be a regular or Free Trial customer with a ThoughtSpot ID and an email address linked to that ID.
...
Custom geo maps are now GA. Upload your own custom maps and visualize data on geo charts with regions that are specific to you and your organization. To try it out, select *Custom maps* in the Admin Console.
...
TML for Monitor alerts is now GA. Export, edit, and import KPI Monitor alerts programmatically, using ThoughtSpot Modeling Language. To try it out, export a Liveboard that contains Monitor alerts and its associated objects.

[#9-1-0-cl-join-tml]
[discrete]
=== Edit join conditions using TML

Use TML to directly edit a join condition by editing the TML file's *on* parameter. This simplifies the previous process of deleting and recreating the join in TML or the ThoughtSpot application. To try it out, edit a TML file, and update a join's *on* parameter.

[#9-1-0-cl-delete-tml]
[discrete]
=== Delete table columns using TML

Remove columns from tables in ThoughtSpot by deleting them in the tables' TML files. To try it out, edit a table TML file, and delete a column's name and properties.
