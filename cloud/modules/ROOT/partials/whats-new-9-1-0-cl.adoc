...
Cross-Region Disaster Recovery allows ThoughtSpot to recover your ThoughtSpot instance in a secondary AWS region in the case of a failure of the primary AWS region where ThoughtSpot is running. This ensures that if one AWS region fails, ThoughtSpot can start in another region, guaranteeing minimal downtime.
