...
We improved the syntax of our text keywords, such as `begins with` and `contains`, to make it easier to choose whether ThoughtSpot combines keyword phrases with *or* logic or *and* logic. You can also add multiple attribute values in one phrase. See xref:keywords.adoc#text[Keyword reference].
