...
Cross-region support allows ThoughtSpot to migrate your ThoughtSpot application from one AWS region to another in the case of AWS region failures. This ensures that if one AWS region fails, ThoughtSpot can start your application in another region, guaranteeing minimal downtime. See xref:business-continuity.adoc[].
