...
To access this feature, go to your KPI chart and select the caret icon to the right of the percent change label. The *Run change analysis* pop-up allows you to customize the columns ThoughtSpot analyzes. Note that you need SpotIQ privileges to use the KPI change analysis feature.
...
Cross-Region Disaster Recovery allows ThoughtSpot to recover your ThoughtSpot cluster in a secondary AWS region in the case of a failure of the primary AWS region where the cluster is running. This ensures that if one AWS region fails, ThoughtSpot can start your application in another region, guaranteeing minimal downtime. See xref:business-continuity.adoc#disaster-recovery[Data Protection and Disaster Recovery].
