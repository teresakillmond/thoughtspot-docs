...
Have you ever tried different chart types and filtering conditions on an answer, and then decided you wanted to set your answer back to its original state? Now you can easily do that by using the new *reset* button image:reset.svg[opts:Inline,20], next to the *undo* and *redo* buttons. This is only available for saved answers in the xref:answer-experience-new.adoc[new answer experience].
