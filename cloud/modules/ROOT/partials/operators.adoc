...
a|
[#if-then-else]
`if...then...else` | Conditional operator | `if ( item type in {'shirts', 'jackets', 'sweatshirts', 'sweaters'}) then 'tops' else if item type in {'shorts', 'pants'}) then 'bottoms' else 'all other apparel'` +
