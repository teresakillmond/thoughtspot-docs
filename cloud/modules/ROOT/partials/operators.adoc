...
a|
[#not-in]
`not in` | Takes a column name and a list of values. It checks each column value against the list of values in the formula, and returns `true` if the column value *_does not_* match one of the values in the formula. | `state not in { 'texas' , 'california' }`
