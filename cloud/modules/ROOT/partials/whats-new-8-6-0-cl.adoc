...
The data panel now supports multi-word search. You can search text and a list of columns separated by one of the following delimiters: comma (*,*), period (*.*), backslash (*\*), dash (*-*), underscore (*_*), or kbd:[*Space*]. If all of your search words find a match with the column name words then that is considered as a match.
...
You can now upload a CSV file that can be joined to a Snowflake connection's tables in your Snowflake database.
