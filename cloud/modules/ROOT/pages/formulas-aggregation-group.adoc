...
* When you create an Answer that contains both a group aggregation function and an `if...then...else` formula, you must specify the aggregation type in the `if...then...else` statement. Otherwise, the default aggregation behavior is `sum`.
