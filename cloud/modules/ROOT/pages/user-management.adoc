...
:description: For each person who accesses ThoughtSpot, you must create a user. When you create a user manually in ThoughtSpot, you manage that user in ThoughtSpot.
...
When you create a user, you can specify the _<<username,username>>_, _<<display-name,display name>>_, _<<sharing-visibility,sharing visibility>>_, _<<email,email>>_, and assign _<<groups,group>>_ memberships.
