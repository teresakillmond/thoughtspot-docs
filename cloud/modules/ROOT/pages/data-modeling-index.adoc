...
To index a column with a cardinality greater than 100,000, set the index type to `PREFIX_ONLY`, `PREFIX_AND_WORD_SUBSTRING`, or `PREFIX_AND_SUBSTRING`. If you must index a column with high cardinality using the `DEFAULT` index type, contact {support-url}.
