...
Use any data source (table, worksheet, or view) that you have access to for the gating condition. For reports sent to ThoughtSpot users, ThoughtSpot executes the query as that specific user to ensure row-level security. For reports sent to people who are not ThoughtSpot users, ThoughtSpot executes the query as the author of the Liveboard schedule.
...
At the scheduled time, ThoughtSpot executes the gating condition query at user level if sending the email to a ThoughtSpot user, or as the author of the schedule if sending the email to someone who is not a ThoughtSpot user.
...
image::gating-condition-pinboard-schedule-example.png[Gating condition Liveboard schedule example]
