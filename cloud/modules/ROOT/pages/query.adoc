...
We count the indexterm2:[row]s of data that can be searched. ThoughtSpot accesses data though connections from external data sources, searchable in TS. Data moves into ThoughtSpot through a connection to an external Cloud data warehouse, or through  CSV upload. Your configuration may label the same data multiple times under different table names, and apply different security rules for different users. We calculate the total row count across all these tables. In ThoughtSpot, we treat views as tables.

Query token: ThoughtSpot search engine indexes both table column names and values of attribute column values. We cache these non-date values locally, and use them for auto-completion when a user types in the search bar. Each cached value is a query token.
