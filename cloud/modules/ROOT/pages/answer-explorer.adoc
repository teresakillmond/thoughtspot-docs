...
. Click on the drop-down under *Select a column to replace*.
. Choose an attribute or measure in your visualization to replace.
. Click on the *And replace with your own column* search bar that appears.
...
. Click the *More* menu icon image:icon-more-10px.png[More menu icon image].
...
Click the *pin* icon image:icon-pin.png[Pin icon image] and select a Liveboard. Note that in Liveboard v2, you see a *Pin* button rather than an icon.
