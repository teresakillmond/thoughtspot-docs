...
* Read the xref:connections-snowflake.adoc#best-practices[best practices for Connections with {connection}].
...
. From the ThoughtSpot home page, a _Connect to {connection}_ guided tutorial appears at the bottom-right of the page.
You can play the video, which shows you the entire workflow of using Connections to connect to {connection}.
. Click *Next*, to be guided, step-by-step, through the process of connecting ThoughtSpot to {connection}, selecting and searching a data source, changing a chart configuration, and more.
