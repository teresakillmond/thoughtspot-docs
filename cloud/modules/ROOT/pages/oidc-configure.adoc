...
[NOTE]
====
If your organization uses xref:orgs-overview.adoc[], ThoughtSpot does not support OIDC integration for authentication. You must use xref:authentication-local.adoc[local authentication] or xref:authentication-integration.adoc[SAML authentication].
====
