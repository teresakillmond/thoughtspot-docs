...
NOTE: The Orgs feature is off by default. To try out this feature, contact {support-url}.
...
WARNING: After you enable the Orgs feature on a cluster, you cannot disable it. However, your environment remains a single-tenant environment until you create an Org, and you can return to single-tenancy by deleting all the Orgs you created.
...
You manage user authentication at the *All orgs* level of the Admin Console. You cannot manage or configure local user authentication to work in different ways for different Orgs in the same cluster.

ThoughtSpot does not currently support SAML authentication for clusters that use Orgs.

For more information about local user authentication, refer to xref:authentication-local.adoc[Managing local authentication].
