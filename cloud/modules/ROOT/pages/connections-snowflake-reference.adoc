...
+
For OAuth with Azure AD provider, it should be similar to `\https://<APPLICATION_ID_URI>/session:role:developer offline_access session:role:developer offline_access`.
For more information, see: xref:connections-snowflake-azure-ad-oauth.adoc#part-1-step-8[part 1, step 8] in Configure Azure AD external OAuth for a {connection} connection.
+
For OAuth with Okta, it should be similar to `session:role:developer offline_access`.
[#oauth-url]
