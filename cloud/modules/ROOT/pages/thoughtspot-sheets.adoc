...
. Analyze your data using the ThoughtSpot search bar, or by clicking items in the data panel.
+
NOTE: ThoughtSpot Connected Sheets adds data starting in the active cell of your Google Sheet. The top-left part of the data table that ThoughtSpot adds to your Sheet appears there. You can control where Connected Sheets adds data by selecting a different cell.  New searches always replace the results of previous searches.

=== Resetting the ThoughtSpot instance URL

If you want to search using a different instance than you initially selected, you must reset the instance URL by doing the following:

. Select *Extensions > ThoughtSpot Connected Sheets > Reset instance URL*.
. Enter the URL of the *ThoughtSpot instance*.
. Click *Continue*.
. Sign in to the instance using your ThoughtSpot username and password, or SSO.
