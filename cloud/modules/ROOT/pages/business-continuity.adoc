...
This article describes ThoughtSpot's disaster recovery strategy for ThoughtSpot Cloud.

There are several methods ThoughtSpot employs to protect your application data and ensure business continuity in the event of a hardware or software failure or a catastrophic event.

== Recovery Point Objective and Recovery Time Objective
A Recovery Point Objective (RPO) is the frequency of which you take a backup of a system. ThoughtSpot's RPO is 24 hours. Every 24 hours, ThoughtSpot automatically takes a backup and stores it in Amazon S3, in a different AWS region than the region your application is in, and in a different Availability Zone.
...
Cross-region support allows ThoughtSpot to migrate your ThoughtSpot application from one AWS region to another in the case of AWS region failures. ThoughtSpot takes <<backups,backups>> of your application once a day, and stores those backups in a different AWS region than the region your application is in. This ensures that if one AWS region fails, ThoughtSpot can start your application in another region, guaranteeing minimal downtime.
