= Managing authentication with SAML using Okta identity and access management
...
[NOTE]
====
This article contains instructions for managing SAML authentication if your company uses ThoughtSpot's Okta identity and access management feature. The Okta feature is in *Beta* and off by default in 8.7.0.cl. If the *SAML* section of the Admin Console is called *SAML integration*, your company is using the Okta feature.

If the *SAML* section of the Admin Console is called *Authentication: SAML*, your company is *_not_* using ThoughtSpot's Okta identity and access management feature. Refer to xref:authentication-integration.adoc[].
====
