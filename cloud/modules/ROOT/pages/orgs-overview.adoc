...
NOTE: The Orgs feature is off by default. To try out this feature, contact {support-url}. To learn about multi-tenancy using groups instead of Orgs, refer to https://developers.thoughtspot.com/docs/?pageid=multi-tenancy-best-practices[Multi-tenancy with groups^].

WARNING: After you enable the Orgs feature on a cluster, you can’t turn off the ability to create Orgs. However, your environment remains a single-tenant environment until you create an Org, and you can return to single-tenancy by deleting all the Orgs you created.
