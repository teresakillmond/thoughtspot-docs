...
This article is an overview of the Orgs functionality and Org management in the standard ThoughtSpot Cloud environment. To learn more about Orgs functionality and programmatic Org management using REST APIs in the ThoughtSpot Everywhere environment, refer to https://visual-embed-sdk-beta-docs.vercel.app/docs/?pageid=orgs[Multi-tenancy with Orgs^]. To learn more about Orgs APIs, refer to https://visual-embed-sdk-beta-docs.vercel.app/docs/?pageid=orgs-api[Orgs API^].
...
Any user with access to multiple Orgs can use the *Org switcher* in the top navigation bar, next to the help icon, to switch between the Orgs they belong to. The *Org switcher* also identifies the Org the user is currently in. For example, in the image below, the user is in the Primary Org.
