...
`<query tokens> (A` *vs* `B) <query tokens>`
...
The first `vs` example compares two segments with a single query token:

image::vs-ex-1.png[]

The system automatically applies the `sales` query token to both sides, and groups each segment.
