...
:linkattrs:
:experimental:
:page-aliases: /admin/loading/use-data-importer.adoc
:description: Use the <code>tsload</code> command to import data from a CSV file.

{% include access.html content="To add data into the system, you must have the required permissions.
...
image::data-tab.png[Click on the Data tab from the top navigation bar]

== ThoughtSpot Loader

To use ThoughtSpot Loader, type the command `tsload` followed by the appropriate flags.
You can see the list of the flags it accepts in the link:{{ site.baseurl }}/reference/data-importer-ref.html#[ThoughtSpot Loader flag reference], or by issuing `tsload -help` on the command line.
...
+
{% include important.html content="--source_file should contain the file path inside the bucket and *_must_* be prefixed by `/s3a/default`.
