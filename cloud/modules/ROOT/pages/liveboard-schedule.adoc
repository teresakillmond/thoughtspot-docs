...
+
image::pinboard-schedule-create.png[Set the Liveboard schedule]
+++<table>++++++<tr>++++++<th>+++Field+++</th>+++
  ...
  +++<p>+++At the scheduled time, ThoughtSpot checks the gating condition, and, if the condition returns true, processes the Liveboard. For a list of valid formulas, see the xref:formula-reference.adoc[Formula function reference].+++</p>+++
  +++<p>+++For an in-depth example of a scenario where a gating condition is useful and powerful, refer to xref:liveboard-gating-condition-example.adoc[Liveboard schedule gating conditions in practice].+++</p>+++
...
include::partial$content/date-column-download.adoc[]
