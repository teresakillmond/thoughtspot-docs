...
If the sub-query is at a less detailed level than the original query, ThoughtSpot adds the result column to the result of the original query.
...
With this feature enabled, users can simply specify the formula as `group_aggregate(count(storeid),query_groups, \{date})`, and the formula will automatically ignore all unrelated filters in the search bar.
