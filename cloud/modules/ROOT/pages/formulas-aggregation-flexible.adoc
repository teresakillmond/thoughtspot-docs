...
Beginning in the 9.0.0.cl release, you can specify a column in the third argument of a group_aggregate function in order to include all filters defined in the search that pertain to that column, and ignore unrelated filters. This feature is in beta and off by default. To enable group aggregation filters, contact ThoughtSpot Support.

Filter condition: `color= 'blue'` `ship mode != 'car'`

For a search on `group_agg color= 'blue'` `ship mode != 'car'`, you can add a formula to calculate the Revenue for all blue-colored items while ignoring the ship mode filter as:

.groupagg
----
group_aggregate (sum(Revenue), {ship mode}, {color, quantity})
----

In this case, the results will be aggregated based on the dimensions in the formula and the filter on the color column; the ship mode not equal to car filter will be ignored, since it is not specified as a filter column in the formula.
