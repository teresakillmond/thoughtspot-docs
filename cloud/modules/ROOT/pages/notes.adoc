= 8.4.0.cl Release Notes
:experimental:
:last_updated: 4/14/2022
:linkattrs:
:page-toclevels: 1
...
ThoughtSpot Cloud 8.4.0.cl is now available.
...
ThoughtSpot now supports the OpenID Connect (OIDC) authentication framework for your ThoughtSpot Cloud instances. OIDC adds an identity layer to the OAuth 2.0 protocol and allows clients to verify a user’s identity based on the authentication performed by an authorization server.
+
NOTE: OIDC Group Sync is not supported in this release.
+
