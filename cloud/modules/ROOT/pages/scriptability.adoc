...
+
* *Ready for import* and *View Warnings*, with a yellow exclamation mark: The object contains errors, but you can still import it. Some visualizations or filters in the Liveboard have errors, the destination tables for some joins in the table do not exist, or the FQNs for the tables or connections referenced in the file do not exist. If you import the object without fixing the errors, the object removes the Liveboard visualizations or filters or table joins. If the FQN contains an error, ThoughtSpot uses name matching to find the table or connection it should use, and updates the FQN with the table or connection's FQN. To view the errors, select *View Warnings*.
