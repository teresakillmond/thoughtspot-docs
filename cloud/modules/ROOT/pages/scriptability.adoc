...
To edit TML for an object using the in-product TML editor, you must have the permissions required to both <<export-permissions,export the object>> and to <<import-permissions,import and update the object>>.
...
. The system displays a *Publish status* dialog.
