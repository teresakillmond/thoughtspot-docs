...
:description: Use Scriptability to export and import Worksheets, views, SQL Views, tables, Liveboards, Monitor alerts, and Answers in a human-readable format.



Use Scriptability to export and import Worksheets, views, tables, Liveboards, Monitor alerts ([.badge.badge-beta]#Beta# and off by default in 9.0.0.cl), and Answers in a human-readable format.

ThoughtSpot developed its own scriptable approach for exporting, enhancing, and migrating Worksheets, views, tables, Liveboards, Monitor alerts ([.badge.badge-beta]#Beta# and off by default in 9.0.0.cl), and Answers.
...
The zip file contains a document called the `Manifest` file, which defines the objects you exported, and their underlying data sources. It also contains the `Alerts.tml` file, which only appears when you export an Answer or Liveboard and its associated objects. The `Alerts.tml` file defines any KPI Monitor alerts. TML for Monitor alerts is in [.badge.badge-beta]#Beta# and off by default in 9.0.0.cl. To enable it, contact {support-url}.
...
. Choose whether to export only the objects, or the objects and their underlying data sources (worksheets, tables, and views). To download any KPI Monitor alerts for the object, you must export the associated objects. The `Alerts.tml` file in the zip file that downloads defines any KPI Monitor alerts. TML for Monitor alerts TML is in [.badge.badge-beta]#Beta# and off by default in 9.0.0.cl. To enable it, contact {support-url}. Choose whether to export the table and connection FQNs. If you select this option, the TML file contains FQNs for the underlying tables and connections.
