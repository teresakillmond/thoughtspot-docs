...
== Sort
To sort any column or row, right-click on a row or column heading or measure or attribute name and select **Sort**. You can also sort from the search bar. Type `sort by <column name> [ascending | descending]`.
...
* When you sort by a measure, you sort rows only. This allows you to sort the rows by a measure, such as `Total sales` in the sample pivot table at the beginning of the article, and the columns by an attribute, such as `Store State` in the sample pivot table at the beginning of the article.
