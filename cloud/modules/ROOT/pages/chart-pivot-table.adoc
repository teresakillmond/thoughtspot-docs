...
== Sort
To sort any column or row, right-click on a row or column heading and select **Sort**.
...
* When you sort by a measure, you sort rows only. This allows you to sort the rows by a measure, like `sales`, and the columns by an attribute, like `date`.
