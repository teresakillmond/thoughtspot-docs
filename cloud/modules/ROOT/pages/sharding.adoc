...
If you use link:{{ site.baseurl }}/data-integrate/embrace/embrace-intro.html[Connections], and store your data in another data warehouse such as Snowflake or Amazon Redshift, your data modeling is done in that data warehouse, and not in ThoughtSpot.
...
To optimize ThoughtSpot performance and memory usage, you should _shard_ very large fact tables whenever possible.
...
The recommended number of shards depends on the number of nodes in the cluster:
...
Instead, you should use `custid` or `locationid`.
+
...
Instead, use `saleid` or `productid` for more variety.
...
This works best if your dimension tables are under 50MB in size.
...
Only shard dimension tables if the dimension table is large (over 50MB), and the join between the fact and dimension tables uses the same columns.
