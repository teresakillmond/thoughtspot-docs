...
You can select a specific worksheet from the *Sources* drop-down to the left of the search bar, or search across all worksheets that you have access to.
...
You can also select these metrics to access the Liveboard they come from, either in the same tab or in a new tab.

To add metrics to your watchlist, follow these steps:

. Select *Add metrics to your watchlist* from the end of the row of existing metrics you are tracking.
+
...
To find metrics from another Liveboard you would like to add, search for that Liveboard using the search bar near the upper left of the *Add metrics* window.
...
* *More options image:icon-more-10px.png[more options menu icon] icon*: You can replace or delete a metric, move it one spot to the left or right, make a card the first one in the row, or insert another card to the right or left of a specific card.
...
Select a tag in the list of objects on the home page, or choose an existing tag from the tag drop-down in the filter bar.
To see objects with any tag or no tag, select *All tags*.
* *By author*: To see objects created by a specific user, select an author from the authors drop-down in the filter bar, or click the author of an object in the list of objects on the home page.
To see objects you created, select your username from the drop-down, right below *All authors*.
...
To bulk mark objects as favorites, click the empty boxes next to the object names, and select *Mark as favorite* near the top of the page, under the metric watchlist.
...
If you select an answer on the first page, and then click *Next* to view more objects, ThoughtSpot's discards your selection on the first page.
