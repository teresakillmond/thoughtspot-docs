...
You can also select these metrics to access the Liveboard they come from, either in the same tab or in a new tab. You can have any number of metrics on your watchlist at a time.
...
NOTE: If you can no longer access a metric on your watchlist (because the KPI chart or Liveboard was deleted, or your access permissions changed), the system displays a note saying you cannot view that metric.

=== Liveboards with mandatory filters
If you want to add a metric from a Liveboard that has a xref:liveboard-filters-mandatory.adoc[mandatory filter], you must first navigate to the Liveboard and specify a value for the mandatory filter. If you do not specify a value, ThoughtSpot doesn't let you add any of the metrics from that Liveboard to your watchlist.
