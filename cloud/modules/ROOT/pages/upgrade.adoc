...
After the upgrade concludes, we will move ThoughtSpot back into read-write mode. That will sign out users again, resulting in a brief disruption of 5 mins or less. Users would then be able to sign in and use the upgraded ThoughtSpot cluster normally. The read-only banner would stop showing, signifying that the system is back to business as usual.
