...
ThoughtSpot sometimes either deprecates or de-supports existing features and replaces them with new and more effective means to use our products.
In this article, we give you advance notice of features we are discontinuing in future releases.

== Definitions
ThoughtSpot sometimes _deprecates_ existing features, and sometimes _de-supports_ them. These terms mean two different things.

Deprecated::
A deprecated feature still exists in the product, for backward compatibility, but there is a plan to remove it in a later release. Deprecated features no longer have official support from ThoughtSpot, but you can still use them.

De-supported::
A de-supported feature does not exist in the product. If you upgrade to a release in which a feature is de-supported, any workflows that rely on that feature will break.
