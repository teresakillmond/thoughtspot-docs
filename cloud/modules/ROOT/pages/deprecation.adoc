...
== Deprecated in 8.10.cl

The 8.10.0.cl release of ThoughtSpot Cloud, targeted for October 2022 (GA), will drop support for the following feature:

Pin Headlines:: ThoughtSpot will deprecate the pin Headline to Liveboard feature. Beginning in the 8.10.cl Cloud release, users will no longer see the pin icon when they click on table summaries. To visualize and pin single aggregated measures, attributes in string/date format, or time-series metrics with sparklines, we recommend creating xref:chart-kpi.adoc[KPI charts]. Existing Headlines pinned to Liveboards will not be affected in this release.
