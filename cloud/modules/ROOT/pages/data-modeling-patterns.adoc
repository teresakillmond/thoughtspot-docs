...
NOTE: If you are formatting percentages, you cannot have a space before the percent sign. #.00% is valid, but #.00 % is not.

=== Profile-based number formatting

Number formatting is set by default based on your ThoughtSpot profile's *System language* setting.
You can set this value to accommodate your geographic locations.

image::profile-locale.png[Set your system language from the profile]
...
You can set a format for how currencies display in tables and charts. This setting affects the ThoughtSpot UI, the Data API, and the UI when embedding ThoughtSpot.
...
See the https://developers.thoughtspot.com/docs/?pageid=rest-apis[Developer documentation^] for more information on the API.

. Find the *Currency Type* for the column whose display format you want to change.
. Select it to open the *Specify Currency Type* menu.
+
image::specify_currency_type.png[Specify the currency type]
