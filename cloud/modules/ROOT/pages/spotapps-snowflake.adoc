...
* Administrator access to {application}. In {application}, this is called `ACCOUNTADMIN`.
...
* *Option 1:* Copy the data from the {application} database to a different database using {application} tasks. This option has faster performance and a customizable cost. Refer to <<option-1,Option 1 SQL commands>>.
...
[#option-2]
==== Option 2
The following SQL commands grant permission for the role you use in your {application} connection to use the {application} database. They prepare you to query on the system database directly. Replace the *database name*, *schema name*, and *YOUR_ROLE* with your specific information.
