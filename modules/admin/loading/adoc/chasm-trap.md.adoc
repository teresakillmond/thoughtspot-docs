...
last_updated: tbd summary: "A chasm trap occurs when two many-to-one joins converge on a single table." sidebar: mydoc_sidebar permalink: /:collection/:path.html -- In a complex schema, you may have a fact table with no relationship to another fact table, except that each contains a foreign key to a shared dimension table.
...
image::{{ site.baseurl }}/images/chasm_trap_complex.png[]
...
* Review the column setting called link:{{ site.baseurl }}/admin/data-modeling/attributable-dimension.html#[Attribution Dimension].
